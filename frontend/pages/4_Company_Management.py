...
with tab_crud:
    ...
    with crud_tabs[3]:
        if companies_df.empty:
            ...
            st.warning("Видалення компанії *не* видалить її кораблі, але вони втратять зв'язок.", icon="⚠️")
        ...
        if current_company_ports_df.empty:
            ...
            if "port_id" not in df.columns:
                if "id" in df.columns:
                    ...
                    # якщо взагалі нічого схожого — показуємо помилку
                    st.error("Очікується, що /api/companies/{id}/ports повертає 'port_id' або 'id'.")
                    st.dataframe(df)
                    st.stop()

            # 2) прапорець головного порту:
            #    бекенд, скоріше за все, віддає 'main' -> робимо 'is_main' для UI
