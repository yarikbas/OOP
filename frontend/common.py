...
def df_1based(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.index.name = "#"   # необов'язково, просто гарний заголовок для індекса
    return df
...
def _after_success(success_msg: str):
    """
    Викликаємо після успішного POST/PUT/DELETE:
    - очищаємо кеш;
    - кладемо повідомлення в session_state;
    - робимо rerun, щоб одразу показати оновлені дані.
    """
    clear_all_caches()
...
def api_post(path: str, payload: dict, success_msg: str):
    """СТВОРЕННЯ (CREATE). Після успіху одразу оновлюємо UI."""
    url = BASE_URL + path
    ...
    _after_success(success_msg)
    return data  # фактично не буде повернуто через st.rerun()


def api_put(path: str, payload: dict, success_msg: str):
    """ОНОВЛЕННЯ (UPDATE). Після успіху одразу оновлюємо UI."""
    url = BASE_URL + path
...
def api_del(path: str, success_msg: str):
    """ВИДАЛЕННЯ (DELETE). Після успіху одразу оновлюємо UI."""
    url = BASE_URL + path
